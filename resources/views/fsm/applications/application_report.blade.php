...
<head>
    <style>
        ...
        .header {
            ...
            text-align: center;
            /* Add margin to the header for separation */
        }
...
<body>
    <div class="container">
        ...
        <table class="table" width="100%" style="margin-top: 20px; border-collapse: collapse;">
            <tr>
                <td style="text-align:left;font-size: 18px; margin: 0; border: none;">{{ __('Application ID') }}:
                    {{$application->id}} </td>
                <td style="float:right;font-size: 18px; margin: 0; border: none;">{{ __('Application Date') }}:
                    {{$application->application_date?->format('Y-m-d')}}</td>
            </tr>
        </table>
    ...
    <table style="width: 100%; letter-spacing: 0.2px">
        <thead>
            <p style="text-align:left;font-size : 19px; font-weight: bold;background-color: #ddddde; padding: 2px"> {{
                __('Building Details') }} </p>
        </thead>
        <tbody>
            <tr>
                ...
                <th style="width: 20%;" rowspan="7">
                    <div style="text-align: center;">
                        <img src="{{'data:image/png;base64,'.base64_encode(file_get_contents(public_path('storage/emptyings/houses/$appplication->emptying->house_image')))}}"
                            height="100px" />
                    </div>
                </th>
                @else
                <th style="width: 20%;" rowspan="8">
                    <div style="text-align: center;">
                        <img src="{{'data:image/png;base64,'.base64_encode(file_get_contents(public_path('img/report-icon/house-image.png')))}}"
    ...
    <table style="width: 100%;">
        ...
        <tbody>
            <tr>
                ...
                <th style="width: 20%;" rowspan="7">
                    <div style="text-align: center;">
                        <img src="{{'data:image/png;base64,'.base64_encode(file_get_contents(public_path('img/report-icon/containment.png')))}}"
                            width="100px" />
                    </div>
                </th>
    ...
    <table style="width: 100%;">
        ...
        <tbody>
            <tr>
                ...
                <th style="width: 20%;" rowspan="7">
                    <div style="text-align: center;">
                        <img src="{{'data:image/png;base64,'.base64_encode(file_get_contents(public_path('img/report-icon/emptying.png')))}}"
                            width="100px" />
                    </div>
                </th>
                ...
                <td style="width: 20%; font-size : 18px;">
                    {{$application->emptying->service_provider()->withTrashed()->first()->company_name?? '-'}}</td>
            </tr>
            <tr>
    ...
    <table style="width: 100%;">
        ...
        <thead>
            <p style="text-align:left;font-size : 19px; font-weight: bold;background-color: #ddddde; padding: 2px">{{
                __('Sludge Collection Details') }} </p>
        </thead>
        <tbody>
            <tr>
                <th style="width: 20%;" rowspan="7">
                    <div style="text-align: center;">
                        <img src="{{'data:image/png;base64,'.base64_encode(file_get_contents(public_path('img/report-icon/sludge.png')))}}"
                            width="100px" />
                    </div>
                </th>
                <td style="width: 20%;  font-size : 18px;">{{ __('Disposal Place') }}</td>
                <td style="width: 20%; font-size : 18px;">{{$application->sludge_collection->treatmentplants->name??
                    '-'}}</td>
            </tr>
            <tr>
            ...
            <tr>
                ...
                <td style="font-size: 18px;">{{ (date('h:i A', strtotime($application->sludge_collection->entry_time))
                    ?? '-') . '-' . (date('h:i A', strtotime($application->sludge_collection->exit_time)) ?? '-') }}
                </td>
            </tr>
        </tbody>
    </table>
